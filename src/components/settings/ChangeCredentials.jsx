...
const ChangeCredentials = () => {
  ...
  return (
    <>
      ...
      <dialog className={`credentials__modal`} ref={modal}>
        <form action="" className={`${isLoading ? 'credentials__center' : ''}`}>
          {!isLoading ? (
            <>
              <label htmlFor="confirm">
                Type your password to change credentials
              </label>
              <input
                value={verify}
                type="text"
                name="confirm"
                id="confirm"
                onChange={(e) => handleInput(e, setVerify)}
              />
              {errors.length > 0 &&
